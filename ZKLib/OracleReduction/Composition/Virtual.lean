...
/-!
  ...
  also need to formalize **virtual** oracle reductions, which are reductions that are defined on
  "virtual" (oracle) statement and witness.

  This is what is meant when we informally say "apply so-and-so protocol to this quantity
  (derived from the input statement & witness)".

  Here are some examples: (TODO: add more?)
